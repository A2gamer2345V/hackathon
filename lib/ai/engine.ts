...
function domainSessions(sessions: GameSession[], domain: CognitiveDomain): GameSession[] {
  return sessions.filter((s) => GAMES_BY_ID[s.gameId]?.category === domain);
}
...
function loadFor(domains: DomainAbility[], sessions: GameSession[]): CognitiveLoad {
  ...
  const recentHints = mean(newestFirst(sessions).slice(0, 6).map((s) => s.hintsUsed));
...
export function configureGame(gameId: GameId, input: EngineInput): GameConfig {
  ...
  const mine = newestFirst(input.sessions.filter((s) => s.gameId === gameId));
  ...
  if (recent.length === 0) {
    ...
    const gap = daysBetween(mine[0].completedAt, input.now ?? new Date());
    if (gap >= 7) {
...
export function gameStats(gameId: GameId, sessions: GameSession[]): GameStats {
  const mine = newestFirst(sessions.filter((s) => s.gameId === gameId));
  ...
  return {
    ...
    trend: trendOf(mine),
    lastPlayedAt: mine[0].completedAt,
  };
}
...
export function buildInsights(input: EngineInput): AiInsight[] {
  ...
  for (const game of GAMES) {
    const mine = newestFirst(input.sessions.filter((s) => s.gameId === game.id));
